...
from core.scheduler import digest_scheduler, PERIOD_BY_SCHEDULE
...
def _build_summary(channels: list, channel_list) -> str:
    ...
    schedule = _schedule_label(channel_list.schedule_type, channel_list.schedule_day, channel_list.schedule_hour, channel_list.schedule_minute)
...
async def cmd_digest(message: types.Message, state: FSMContext) -> None:
    ...
    if ch_list and channels:
        ...
        await message.answer(
            ...
            reply_markup=digest_first_keyboard(),
        )
        await state.set_state(DigestStates.choosing_list)
...
# ─── Шаг 1: выбор списка каналов (первый запуск) ─────────

@router.callback_query(DigestStates.choosing_list, F.data.startswith("digest_list:"))
async def on_choose_list(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    async with async_session() as session:
        if choice == "default":
            ...
            if not channels:
                await state.update_data(channel_list_id=ch_list.id, channel_ids=[], channel_names=[])
                await callback.message.edit_text(
            ...
            await _go_to_keywords(callback.message, state, session, [c.id for c in channels])
            return
...
async def on_editing_remove(callback: types.CallbackQuery, state: FSMContext) -> None:
    data = await state.get_data()
    names_str = " · ".join(f"@{n}" for n in data.get("channel_names", [])) or "список пуст"
    await callback.message.edit_text(
...
async def on_remove_channels(message: types.Message, state: FSMContext) -> None:
    parts = message.text.strip().split()
    to_remove = {p.lstrip("@").split("t.me/")[-1].strip("/").lower() for p in parts if p}
...
async def _go_to_keywords(message, state, session, channel_ids: list[int]) -> None:
    ...
    await message.edit_text(
        "🔑 Выберите темы для фильтрации или нажмите «Без фильтра»:",
        reply_markup=keywords_keyboard(topics_list),
    )
...
async def on_keyword(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    if value == "custom":
        await callback.message.edit_text("✍️ Введите ключевое слово:")
...
async def _save_filter_and_return_to_edit(callback: types.CallbackQuery, state: FSMContext) -> None:
    data = await state.get_data()
    list_id = data.get("channel_list_id")
        ...
        async with async_session() as session:
            ...
            if ch_list:
                await update_channel_list_schedule(
                    session, list_id,
                    ch_list.schedule_type, ch_list.schedule_day, ch_list.schedule_hour,
...
async def on_schedule(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    await state.update_data(schedule_type=schedule_type if schedule_type != "none" else None)
...
@router.callback_query(DigestStates.choosing_schedule_day, F.data.startswith("schedule_day:"))
...
@router.callback_query(DigestStates.choosing_schedule_time, F.data.startswith("schedule_time:"))
async def on_schedule_time(callback: types.CallbackQuery, state: FSMContext) -> None:
    await state.update_data(schedule_hour=int(callback.data.split(":")[1]), schedule_minute=0)
    await _show_confirmation(callback, state)
    await callback.answer()
...
async def on_schedule_time_text(message: types.Message, state: FSMContext) -> None:
    ...
    if parsed is None:
        await message.answer("Не распознал время. Введите в формате <b>ЧЧ:ММ</b>, например: 9:00 или 23:30")
        return
    hour, minute = parsed
...
async def on_confirm(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    if list_id:
        async with async_session() as session:
            await update_channel_list_schedule(
                session, list_id, schedule_type, schedule_day, schedule_hour,
